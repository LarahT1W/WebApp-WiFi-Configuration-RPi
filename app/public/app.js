...
app.controller("AppController", ["PiManager", "$scope", "$location", "$timeout",
    ...
    function(PiManager, $scope, $location, $timeout) {
        ...
        $scope.change_selection = function(cell) {
            ...
            var pop_up = document.getElementsByClassName("rwc-password-entry-container")[0];
            if(!angular.element(pop_up).hasClass("show")) {
                angular.element(pop_up).addClass("show");
            }
        }
...
//                return $http.post("/api/enable_wifi", wifi_info);
                    $http.post("/api/enable_wifi", wifi_info);
    ...
    return {
        ...
        link: function(scope, element, attributes) {
            scope.reset = function () {
                var pop_up = document.getElementsByClassName("rwc-password-entry-container")[0];
                angular.element(pop_up).removeClass("show");
            }
        },
    };
});
...
/*-------------------- Rotate ------------------*/
var rotate = document.getElementsByClassName(".establishing a i");

if(!angular.element(rotate).hasClass("fa-spin2")){
    angular.element(rotate).addClass("fa-spin2");
}

/*-------------------- Hide/Show Form------------------*/
/*$(document).ready(function(){

    var pop_up = $(".rwc-password-entry-container");
    var wifi_list = $(".scan-result");
